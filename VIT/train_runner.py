...
class TrainRunner:
    ...
    def train(
        self, model: nn.Module, lr: float, bs: int, epochs: int, name = None
    ):
        g = torch.Generator().manual_seed(42)
        train_data, val_data = random_split(
            self.train_data, [50000, 10000], generator=g
        )
        ...
        for epoch in range(1, epochs + 1):
            ...
            print(
                f"epoch {epoch} | loss: {train_loss:.4f}, acc: {train_acc:.4f} loss (val):{val_loss:.4f}, acc (val): {val_acc:.4f}"
            )
